...
void Projectile::init()
{
    ...
    if (pb != nullptr) {
        ...
        spdlog::critical("GameObject {} has a Projectile so it must also have a PhysicsBody", this->m_parent->getId());
    }
...
void Projectile::update(const float& dt, const float& elapsedTime)
{
...
void Projectile::copy(std::shared_ptr<Component> otherCompPtr)
{
    std::shared_ptr<Projectile> other = std::dynamic_pointer_cast<Projectile>(otherCompPtr);

    *this = *other;
}

void Projectile::loadDefinition(nlohmann::json object)
{
...
void Projectile::setOwner(std::shared_ptr<GameObject> owner)
{

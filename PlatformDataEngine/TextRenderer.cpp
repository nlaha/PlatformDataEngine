...
void PlatformDataEngine::TextRenderer::init()
{
}

void PlatformDataEngine::TextRenderer::update(const float& dt, const float& elapsedTime)
{
}

void PlatformDataEngine::TextRenderer::draw(sf::RenderTarget& target, sf::RenderStates states) const
{
	...
	if (this->m_text.size() > 0) {
		...
		{
			sf::Glyph prevGlyph = this->m_font->getGlyph(this->m_text[(i - 1) < 0 ? 0 : i - 1], this->m_fontSize, this->m_fontBold);
...
void PlatformDataEngine::TextRenderer::loadDefinition(nlohmann::json object)
{
	this->m_font = std::make_shared<sf::Font>();
	m_font->loadFromFile(object.at("fontPath"));
